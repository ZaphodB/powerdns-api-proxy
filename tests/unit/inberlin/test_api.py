...
def test_zone_delete_rollback_recreates(client, fake_pdns):
    ...
    entry = client.get("/proxy/v1/journal", headers=act_as("alice")).json()["entries"][
        0
    ]
...
def test_create_override_duplicate_409(client):
    ...
    assert (
        client.post("/proxy/v1/overrides", headers=admin, json=body).status_code == 201
    )
    # UNIQUE(zone) violation must surface as a conflict, not a 500
    assert (
        client.post("/proxy/v1/overrides", headers=admin, json=body).status_code == 409
    )
